...
@app.route('/')
def index():
    cursor = g.db.execute('select name, title, content, posts.id, authors.id from authors, posts where authors.id = author_id')
...
def getPost(post_id=None):
    if not post_id:
        abort(400)
    cursor = g.db.execute('select name, title, content, posts.id, authors.id from authors, posts where posts.id = ? and authors.id = author_id', [post_id])
    posts = [dict(author_name=row[0], title=row[1], content=row[2], id=row[3], author_id=row[4]) for row in cursor.fetchall()]
    if len(posts) == 0:
        abort(404)
    return render_template('index.html', posts=posts)
...
def getAuthorPosts(author_id=None):
    if not author_id:
        abort(400)
    cursor = g.db.execute('select name, title, content, posts.id, authors.id from authors, posts where author_id = authors.id and posts.author_id = ?', [author_id])
    posts = [dict(author_name=row[0], title=row[1], content=row[2], id=row[3], author_id=row[4]) for row in cursor.fetchall()]
    if len(posts) == 0:
        abort(404)
    return render_template('index.html', posts=posts)
...
def getCategoryPosts(cat_id=None):
    if not cat_id:
        abort(400)
    cursor = g.db.execute('select name, title, content, posts.id, authors.id from authors, posts where posts.author_id = authors.id and posts.category_id = ?', [cat_id])
    posts = [dict(author_name=row[0], title=row[1], content=row[2], id=row[3], author_id=row[4]) for row in cursor.fetchall()]
    if len(posts) == 0:
        abort(404)
    return render_template('index.html', posts=posts)
...
def create():
    ...
    if not logged:
        return redirect(url_for('adminLoggingPage'))
    cursor = g.db.execute('select name, id from authors')
    authors = [dict(name=row[0], id=row[1]) for row in cursor.fetchall()]
    cursor = g.db.execute('select name, id from categories')
...
def createPost():
    ...
    g.db.execute('insert into posts (author_id, category_id, title, content) values (?, ?, ?, ?)', [aut_id, cat_id, post_title, text])
    g.db.commit()
    return redirect("/")
